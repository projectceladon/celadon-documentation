...
Stable Releases (IoT) build running in GVT-d mode has been validated on Intel TGL/EHL/CML in the following function domains. Validation cycles are performed on GMS user signed widevine enabled image as required by google certification requirements
